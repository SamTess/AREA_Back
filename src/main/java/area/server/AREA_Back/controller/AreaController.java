...
public class AreaController {
    ...
    public ResponseEntity<List<AreaResponse>> getAreasByUserId(@PathVariable Long userId) {
    ...
    public ResponseEntity<AreaResponse> createArea(@Valid @RequestBody CreateAreaRequest request) {
        ...
        Optional<Service> actionService = serviceRepository.findById(request.getActionServiceId());
        if (!actionService.isPresent())
            return ResponseEntity.badRequest().build();

        Optional<Service> reactionService = serviceRepository.findById(request.getReactionServiceId());
        if (!reactionService.isPresent())
            return ResponseEntity.badRequest().build();
        ...
        area.setActionService(actionService.get());
        area.setActionType(request.getActionType());
        area.setActionConfig(request.getActionConfig());
        area.setReactionService(reactionService.get());
        area.setReactionType(request.getReactionType());
        area.setReactionConfig(request.getReactionConfig());
    ...
    public ResponseEntity<AreaResponse> updateArea(
        ...
        if (!actionService.isPresent() || !reactionService.isPresent())
            return ResponseEntity.badRequest().build();
    ...
    public ResponseEntity<AreaResponse> toggleArea(@PathVariable Long id) {
        Optional<Area> optionalArea = areaRepository.findById(id);
        if (!optionalArea.isPresent()) {
            return ResponseEntity.notFound().build();
        }
    ...
    private AreaResponse convertToResponse(Area area) {
        ...
        response.setUserUsername(area.getUser().getUsername());
        response.setActionServiceId(area.getActionService().getId());
        response.setActionServiceName(area.getActionService().getName());
        response.setActionType(area.getActionType());
        response.setActionConfig(area.getActionConfig());
        response.setReactionServiceId(area.getReactionService().getId());
        response.setReactionServiceName(area.getReactionService().getName());
        response.setReactionType(area.getReactionType());
        response.setReactionConfig(area.getReactionConfig());
        response.setLastTriggered(area.getLastTriggered());
